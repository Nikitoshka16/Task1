...
def load_initial_student_data(conn):
    ...
    SELECT DISTINCT ON (userid, courseid) 
        ...
        CAST(NameR_Level AS INTEGER) -- Явно приводим VARCHAR к INTEGER
    FROM public.user_logs
    WHERE NameR_Level IS NOT NULL 
      AND NameR_Level ~ '^[0-9]+$' -- Дополнительная защита: берем только те строки, где реально лежат цифры
...
def calculate_advanced_metrics(conn):
    # Среднее количество событий в неделю. Делим общую сумму на количество недель
    query_avg_events = """
    UPDATE dmr.analytics_student_performance AS target
    SET 
        avg_weekly_events = ROUND(
            ...
            2
        ) -- Вот здесь закрыли скобку функции ROUND
    FROM (
    ...
    # Пиковая неделя активности. Временная таблица для поиска лучшей недели
    query_peak_week = """
    WITH RankedWeeks AS (
        SELECT 
            userid, 
    ...
    # Коэффициент стабильности. Вычисляем отношение активных недель к общему количеству недель
    query_consistency = """
    UPDATE dmr.analytics_student_performance AS target
    SET 
    ...
    # Категория активности. Сравниваем студентов между собой и делим на группы
    query_category = """
    ...
    conn.commit()
    print("Продвинутые аналитические метрики успешно рассчитаны.")
